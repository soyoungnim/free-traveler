...
- **Priority:** P0
- **소스:** `TASKS/00_TASK_LIST.md` Seq 14

## Context

**CMP-SCR002-TIMELINE — 여행 Timeline** (Component 구현). Screen `SCR-002`(Route `/about`)의 일부로 동작한다. 우선순위 P0, Depends On: DATA-REPRESENTATIVE.
...
`src/app/_components/scr002/Timeline.tsx`(create)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

연도·장소·한 줄 요약 항목 6개 이상 시간순; 최소 6개 미만이면 게시 차단(데이터 게이트)

## Visual AC

Timeline 세로/가로 레이아웃, 다른 Section과 시각적으로 구분
...
- TC1: 여행 Timeline의 Functional AC에 기술된 각 동작이 지정된 입력에 대해 명세대로 동작한다.
