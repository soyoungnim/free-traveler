# CMP-GLOBAL-SEO-META — 공개 페이지 메타데이터

- **Category:** COMPONENT
- **Implementation Status:** IMPLEMENT
- **Priority:** P1
- **소스:** `TASKS/00_TASK_LIST.md` Seq 33

## Context

**CMP-GLOBAL-SEO-META — 공개 페이지 메타데이터** (Component 구현). 특정 Screen에 종속되지 않는 공용/기반 Task다. 우선순위 P1, Depends On: -.
...
- `design-reference/D-001/DESIGN.md` § Do / Do Not(미등록 색상·상표 금지 외 해당 없음 — 메타데이터는 비시각 요소)
...
`src/app/page.tsx`, `src/app/about/page.tsx` 등 각 `page.tsx`(modify, `generateMetadata`)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

title/description/canonical/Open Graph 페이지별 고유 값
...
- TC1: 공개 페이지 메타데이터의 Functional AC에 기술된 각 동작이 지정된 입력에 대해 명세대로 동작한다.
