...
- **Priority:** P1
- **소스:** `TASKS/00_TASK_LIST.md` Seq 58

## Context

**UNIT-CONTACT-DETECTION — 연락처 탐지 Unit Test** (Unit Test 구현). 특정 Screen에 종속되지 않는 공용/기반 Task다. 우선순위 P1, Depends On: CMP-SCR003-MATE-TAB.
...
`src/app/_components/scr003/__tests__/contact-detection.test.ts`(create)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

전화번호·이메일·메신저 ID 패턴 기준 테스트셋 탐지율 95%+, 오탐 5% 이하 검증
