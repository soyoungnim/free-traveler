# CMP-SCR004-REPORT-BLOCK — 신고·차단 버튼/폼

- **Category:** COMPONENT
- **Implementation Status:** IMPLEMENT
- **Priority:** P0
- **소스:** `TASKS/00_TASK_LIST.md` Seq 25

## Context

**CMP-SCR004-REPORT-BLOCK — 신고·차단 버튼/폼** (Component 구현). Screen `SCR-004`(Route `/mates`)의 일부로 동작한다. 우선순위 P0, Depends On: API-MODERATION.
...
`src/app/_components/scr004/ReportBlockActions.tsx`(create)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

사유 코드+설명 신고 제출, 접수 ID 표시, 차단/차단 해제

## Visual AC

안전 안내 CTA Banner("여행 준비 이어가기" → SCR-003)

## Security/Privacy AC

신고자·피신고자 상세는 관리자만 접근, 차단 후 상호 콘텐츠 미노출

## Test Cases

- TC1: 신고·차단 버튼/폼의 Functional AC에 기술된 각 동작이 지정된 입력에 대해 명세대로 동작한다.
