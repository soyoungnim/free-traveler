# CI-BUILD-LINT-GATE — TypeScript strict·ESLint 빌드 게이트

- **Category:** CI_OPS
- **Implementation Status:** IMPLEMENT(축소)
- **Priority:** P1
- **소스:** `TASKS/00_TASK_LIST.md` Seq 53

## Context

**CI-BUILD-LINT-GATE — TypeScript strict·ESLint 빌드 게이트** (CI/운영 스크립트 구현). 특정 Screen에 종속되지 않는 공용/기반 Task다. 우선순위 P1, Depends On: -.
...
`tsconfig.json`(modify, strict), `.github/workflows/*` 또는 Vercel 빌드 설정(modify)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

`npm run build`, `npm run lint` 병합 전 통과 강제
