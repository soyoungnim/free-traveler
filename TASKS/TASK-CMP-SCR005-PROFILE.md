...
- **Priority:** P0
- **소스:** `TASKS/00_TASK_LIST.md` Seq 27

## Context

**CMP-SCR005-PROFILE — 프로필·성인 확인** (Component 구현). Screen `SCR-005`(Route `/account`)의 일부로 동작한다. 우선순위 P0, Depends On: API-ACCOUNT.
...
`src/app/_components/scr005/ProfilePanel.tsx`(create)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

닉네임·연령대·스타일 필수, 성별 선택, 성인 확인 절차(배지: 완료/필요)

## Visual AC

배지는 `color.success`/`color.warning` + 텍스트
...
- TC1: 프로필·성인 확인의 Functional AC에 기술된 각 동작이 지정된 입력에 대해 명세대로 동작한다.
