# CMP-GLOBAL-AUTH-CALLBACK — Supabase 이메일 인증 콜백 Route

- **Category:** COMPONENT
- **Implementation Status:** IMPLEMENT
- **Priority:** P0
- **소스:** `TASKS/00_TASK_LIST.md` Seq 37

## Context

**CMP-GLOBAL-AUTH-CALLBACK — Supabase 이메일 인증 콜백 Route** (Component 구현). 특정 Screen에 종속되지 않는 공용/기반 Task다. 우선순위 P0, Depends On: DB-SCHEMA-BASE.
...
이메일 인증 콜백 처리 후 `/account`로 리다이렉트
...
콜백 코드 1회성 검증, 세션 쿠키 SameSite 설정

## Test Cases

- TC1: Supabase 이메일 인증 콜백 Route의 Functional AC에 기술된 각 동작이 지정된 입력에 대해 명세대로 동작한다.
