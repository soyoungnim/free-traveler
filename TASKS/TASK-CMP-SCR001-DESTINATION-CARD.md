# CMP-SCR001-DESTINATION-CARD — 여행지 카드(즐겨찾기 포함)

- **Category:** COMPONENT
- **Implementation Status:** IMPLEMENT(축소)
- **Priority:** P0
- **소스:** `TASKS/00_TASK_LIST.md` Seq 7

## Context

**CMP-SCR001-DESTINATION-CARD — 여행지 카드(즐겨찾기 포함)** (Component 구현). Screen `SCR-001`(Route `/`)의 일부로 동작한다. 우선순위 P0, Depends On: DATA-DESTINATIONS.
...
`src/app/_components/scr001/DestinationCard.tsx`(create)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

카드 클릭 시 상세 Drawer 오픈 트리거, 즐겨찾기 토글(중복 방지); 내용 없는 빈 Card 금지

## Visual AC

4:3 이미지 `next/image` lazy load, 12px radius, alt는 실제 장소 설명 문장

## Security/Privacy AC

즐겨찾기는 `localStorage`에만 저장, 서버/DB 미전송

## Test Cases

- TC1: 여행지 카드(즐겨찾기 포함)의 Functional AC에 기술된 각 동작이 지정된 입력에 대해 명세대로 동작한다.
