# API-ACCOUNT — 프로필·성인확인·탈퇴

- **Category:** API
- **Implementation Status:** IMPLEMENT(축소)
- **Priority:** P0
- **소스:** `TASKS/00_TASK_LIST.md` Seq 49

## Context

**API-ACCOUNT — 프로필·성인확인·탈퇴** (서버 접근 로직(Server Action) 구현). 특정 Screen에 종속되지 않는 공용/기반 Task다. 우선순위 P0, Depends On: DB-ACCESS, DB-RLS-BASE.
...
REQ-FUNC-028,029,045,066; REQ-NF-018
...
`src/lib/db/account.ts`(create)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

성인확인 상태·시각 기록, 프로필 갱신, 탈퇴 시 즉시 비식별화
