# CMP-GLOBAL-A11Y — 접근성 공통 컴포넌트(포커스·ARIA)

- **Category:** COMPONENT
- **Implementation Status:** IMPLEMENT
- **Priority:** P1
- **소스:** `TASKS/00_TASK_LIST.md` Seq 32

## Context

**CMP-GLOBAL-A11Y — 접근성 공통 컴포넌트(포커스·ARIA)** (Component 구현). 특정 Screen에 종속되지 않는 공용/기반 Task다. 우선순위 P1, Depends On: CMP-GLOBAL-DESIGN-TOKENS.
...
`src/app/_components/a11y/FocusRing.tsx`(create)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

폼·모달·탭·알림에 올바른 HTML 시맨틱+ARIA, `:focus-visible` 2px 아웃라인, 44px 터치 영역

## Visual AC

색상 단독 상태 구분 금지, 텍스트 라벨 병기
...
- TC1: 접근성 공통 컴포넌트(포커스·ARIA)의 Functional AC에 기술된 각 동작이 지정된 입력에 대해 명세대로 동작한다.
