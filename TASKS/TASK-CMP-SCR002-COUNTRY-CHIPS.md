...
- **Priority:** P0
- **소스:** `TASKS/00_TASK_LIST.md` Seq 15

## Context

**CMP-SCR002-COUNTRY-CHIPS — 방문 국가 Chip(권역별)** (Component 구현). Screen `SCR-002`(Route `/about`)의 일부로 동작한다. 우선순위 P0, Depends On: DATA-REPRESENTATIVE.
...
`src/app/_components/scr002/VisitedCountryChips.tsx`(create)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

권역 그룹 헤더 + 국가 Chip 30개 이상; 30개 미만이면 게시 차단

## Visual AC

Chip 기본 `surface-strong`, 코랄 미사용(선택 상태 없음)
...
- TC1: 방문 국가 Chip(권역별)의 Functional AC에 기술된 각 동작이 지정된 입력에 대해 명세대로 동작한다.
