# API-MATES-APPLICATIONS — 참가 요청 제출·승인·거절

- **Category:** API
- **Implementation Status:** IMPLEMENT
- **Priority:** P0
- **소스:** `TASKS/00_TASK_LIST.md` Seq 47

## Context

**API-MATES-APPLICATIONS — 참가 요청 제출·승인·거절** (서버 접근 로직(Server Action) 구현). 특정 Screen에 종속되지 않는 공용/기반 Task다. 우선순위 P0, Depends On: DB-ACCESS, DB-RLS-BASE.
...
`src/lib/db/mate-applications.ts`(create)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

중복 PENDING/ACCEPTED 차단(DB unique 제약), 작성자만 승인/거절 전이 허용, 상태 변경 시 알림 트리거
