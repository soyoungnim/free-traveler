...
- **Priority:** P1
- **소스:** `TASKS/00_TASK_LIST.md` Seq 57

## Context

**UNIT-TRAVEL-DATES — 날짜 검증 Unit Test** (Unit Test 구현). 특정 Screen에 종속되지 않는 공용/기반 Task다. 우선순위 P1, Depends On: CMP-SCR003-FLIGHT-TAB, CMP-SCR003-HOTEL-TAB, CMP-SCR004-FILTER-LIST.
...
CMP-SCR003-FLIGHT-TAB, CMP-SCR003-HOTEL-TAB, CMP-SCR004-FILTER-LIST

## Expected Files

`src/app/_components/scr003/__tests__/dates.test.ts`(create)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

과거일·역전일·경계값(체크인=체크아웃) 차단, 동행글 종료일 경과 시 CLOSED 판정 로직 검증
