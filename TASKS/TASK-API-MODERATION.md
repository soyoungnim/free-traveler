# API-MODERATION — 신고 접수·상태 처리·차단

- **Category:** API
- **Implementation Status:** IMPLEMENT
- **Priority:** P0
- **소스:** `TASKS/00_TASK_LIST.md` Seq 48

## Context

**API-MODERATION — 신고 접수·상태 처리·차단** (서버 접근 로직(Server Action) 구현). 특정 Screen에 종속되지 않는 공용/기반 Task다. 우선순위 P0, Depends On: DB-ACCESS, DB-RLS-BASE.
...
REQ-FUNC-039,040,041,042; REQ-NF-019
...
`src/lib/db/moderation.ts`(create)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

신고 접수 ID 생성, OPEN→RESOLVED/DISMISSED 전이, 차단/차단 해제
