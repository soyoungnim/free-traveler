# CMP-SCR002-RECOMMENDED-DEST-CTA — 추천 여행지 4개 + 문의/SNS + CTA

- **Category:** COMPONENT
- **Implementation Status:** IMPLEMENT
- **Priority:** P0
- **소스:** `TASKS/00_TASK_LIST.md` Seq 17

## Context

**CMP-SCR002-RECOMMENDED-DEST-CTA — 추천 여행지 4개 + 문의/SNS + CTA** (Component 구현). Screen `SCR-002`(Route `/about`)의 일부로 동작한다. 우선순위 P0, Depends On: DATA-REPRESENTATIVE, DATA-DESTINATIONS.
...
`src/app/_components/scr002/RecommendedDestinations.tsx`(create)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

추천 여행지 카드 4개(→SCR-001 상세 Drawer 연결), 문의·SNS 링크(빈 값 생략), CTA Banner("여행 준비 시작하기"/"동행 찾아보기"); 빈 링크는 렌더링 생략(빈 자리 아님)
...
- TC1: 추천 여행지 4개 + 문의/SNS + CTA의 Functional AC에 기술된 각 동작이 지정된 입력에 대해 명세대로 동작한다.
