# CMP-SCR005-ADMIN — 신고 상태 처리·외부 URL 설정

- **Category:** COMPONENT
- **Implementation Status:** IMPLEMENT(축소)
- **Priority:** P0
- **소스:** `TASKS/00_TASK_LIST.md` Seq 29

## Context

**CMP-SCR005-ADMIN — 신고 상태 처리·외부 URL 설정** (Component 구현). Screen `SCR-005`(Route `/account`)의 일부로 동작한다. 우선순위 P0, Depends On: API-MODERATION, API-ADMIN-SETTINGS.
...
API-MODERATION, API-ADMIN-SETTINGS

## Expected Files

`src/app/_components/scr005/AdminPanel.tsx`(create)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

신고 목록(OPEN/RESOLVED/DISMISSED 필터), 상태 변경, 항공·숙소 외부 URL HTTPS 허용목록 설정; 신고 0건 시 안내 문구만(액션 불필요)
...
- TC1: 신고 상태 처리·외부 URL 설정의 Functional AC에 기술된 각 동작이 지정된 입력에 대해 명세대로 동작한다.
