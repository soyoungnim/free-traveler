# API-ADMIN-SETTINGS — 외부 URL 설정

- **Category:** API
- **Implementation Status:** IMPLEMENT
- **Priority:** P0
- **소스:** `TASKS/00_TASK_LIST.md` Seq 50

## Context

**API-ADMIN-SETTINGS — 외부 URL 설정** (서버 접근 로직(Server Action) 구현). 특정 Screen에 종속되지 않는 공용/기반 Task다. 우선순위 P0, Depends On: DB-ACCESS, DB-RLS-BASE.
...
`src/lib/db/admin-settings.ts`(create)

> 이 Task는 위에 나열된 파일 외 어떤 파일도 생성·수정하지 않는다.

## Functional AC

항공·숙소 외부 URL을 HTTPS 허용목록 내에서만 저장
...
HTTP·javascript·data URL 저장 거부, Admin 역할만 쓰기 허용
...
- [ ] Test Cases 전항목 통과
- [ ] Verify에 명시된 Task(TEST-RLS-BASIC, E2E-MATE-AUTH)와 연동 확인
